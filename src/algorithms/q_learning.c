...
QLearning* ql_create(State states, Action actions, double alpha, double gamma, double epsilon) {
    ...
    if(ql == NULL) {
        ...
        println("--- Q-Learning ---");
        println("Successfully created Q-Learning object!");
    }
...
int ql_get_action(QLearning *ql) {
    if(get_random_float() < ql->epsilon) {
        ...
        // int best_action = 0;
        // double max_q = ql->q_table[ql->states.state][0];
        // for(int a = 1; a < ql->action_size; a++) {
        //     if(ql->q_table[ql->states.state][a] > max_q) {
        //         max_q = ql->q_table[ql->states.state][a];
        //         best_action = a;
        //     }
        // }
        // return best_action;
